...
#Accessed on Friday, 18 Jan, 2018

#Create Data Frame
wash_monument <- read.table(text = 'Year,RecreationVisitors,TotalRecreationVisitors
                            ...
                            2016,"252,677","81,365,896"', sep = ',', header = TRUE)

#Clean the data (Take out commas and change to 'in millions')
wash_monument$RecreationVisitors <- as.numeric(gsub(',','', wash_monument$RecreationVisitors))/10^6
tail(wash_monument)
#EDA Plot the time series
plot(RecreationVisitors~Year, data = wash_monument, type = 'b', ylab = 'Washington Monument Annual Visitors (In Millions)')
...
#Date Accessed: Friday, 18 Jan, 2018

#Create Data Frame
jeff_mem <- read.table(text='Year,RecreationVisitors,TotalRecreationVisitors
                       1941,0,"107,660,113"
                       ...
                       2016,"3,414,345","107,660,113"', sep = ',', header = TRUE)

#Clean the Data (Take out commas and convert to "in millions")
jeff_mem$RecreationVisitors <- as.numeric(gsub(',','',jeff_mem$RecreationVisitors))/10^6
tail(jeff_mem)
#EDA plot the time series
plot(RecreationVisitors~Year, data = jeff_mem, type = 'b', ylab = "Jefferson Memorial Annual Visitors (In millions)")

#Wright Brothers Memorial Problem
#Title,Bookmark
...
#Create Data Frame
Wright_mem <- read.table(text='Year,RecreationVisitors,TotalRecreationVisitors
                         ...
                         2016,"458,776","27,109,570"', sep = ',', header = TRUE)

#Clean the Data (Remove commas and change to "in millions")
Wright_mem$RecreationVisitors <- as.numeric(gsub(',','',Wright_mem$RecreationVisitors))/10^6
tail(Wright_mem)
#EDA plot time series
plot(RecreationVisitors~Year, data = Wright_mem, type='b', ylab = 'Wright Brothers Memorial Anual Visitors (In millions)')
...
#Create Data Frame
Acadia <- read.table(text='Year,RecreationVisitors,TotalRecreationVisitors
                     ...
                     2016,"3,303,393","163,512,192"', sep=',', header=TRUE)
#Clean Data by removing commas and changing to "in millions"
Acadia$RecreationVisitors <- as.numeric(gsub(',','',Acadia$RecreationVisitors))/10^6
tail(Acadia)
#EDA plot time series
plot(RecreationVisitors~Year, data = Acadia, type='b', ylab = "Acadia Annual Visitors (In millions)")

#Question 2 on the Lincoln Memorial
#Title,Bookmark
#Lincoln Memorial,Bookmark this report: https://irma.nps.gov/Stats/SSRSReports/Park%20Specific%20Reports/Annual%20Park%20Recreation%20Visitation%20(1904%20-%20Last%20Calendar%20Year)

#Date Accessed: Thursday, 18 Jan, 2018

#Create Data Frame
lincoln_mem <- read.table(text='Year,RecreationVisitors,TotalRecreationVisitors
                          ...
                          2016,"7,915,934","238,620,382"',sep=',',header=TRUE)

#Clean Data by removing commas and changing to "in millions"
lincoln_mem$RecreationVisitors <- as.numeric(gsub(',','',lincoln_mem$RecreationVisitors))/10^6
tail(lincoln_mem)
#EDA
plot(RecreationVisitors~Year, data = lincoln_mem, type='b', ylab = 'Lincoln Memorial Annual Visitors (In Millions)')

#Model ARIMA(1,1,1)

#Fit Model
library(astsa)
#Find Parameter Estimates and SE's
lincoln_mem.out <- sarima(lincoln_mem$RecreationVisitors,1,1,1)
lincoln_mem.out$ttable

#Predict next five years of attendance
lincoln_mem.future <- sarima.for(lincoln_mem$RecreationVisitors,n.ahead = 5,1,1,1)

#Compute 95% Confidence Intervals
lower <- lincoln_mem.future$pred - 2*lincoln_mem.future$se
upper <- lincoln_mem.future$pred + 2*lincoln_mem.future$se
cbind(lincoln_mem.future$pred, lower, upper)

#Graphic
plot(RecreationVisitors~Year, data = lincoln_mem, type = 'b', ylab="Lincoln Memorial Annual Visitors (in millions)",
     xlim=c(1937,2021),ylim=c(0,12))
lines(2017:2021,lincoln_mem.future$pred,col="darkorange2",type="b",pch=19)
lines(2017:2021,lower,col="darkorange2",lty=2)
lines(2017:2021,upper,col="darkorange2",lty=2)